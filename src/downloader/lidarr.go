...
func (c *Lidarr) MoveDownload(srcDir, destDir, albumPath string, track *models.Track) (string, error) {
	...
	for _, file := range albumFiles {
		...
		if _, err := moveTrack(srcFile, destDir, &albumTrack, c.Cfg.PathTemplate, c.Cfg.KeepPermissions); err != nil {
